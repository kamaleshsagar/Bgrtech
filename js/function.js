...
/* Read more JS*/
    ...
    function toggleReadMoreButtons() {
        document.querySelectorAll('.row.pb-50').forEach(row => { // Target correct class
            let imageContainer = row.querySelector('.image-container');
            let textContainer = row.querySelector('.text-container');
            let readMoreBtn = row.querySelector('.toggle-btn1'); // Adjusted class name

            // Ensure all elements exist before proceeding
            if (!imageContainer || !textContainer || !readMoreBtn) return;

            let textActualHeight = textContainer.scrollHeight; // Get full height of text
            let imageActualHeight = imageContainer.offsetHeight; // Get full height of image

            // console.log("Text Height:", textActualHeight, "Image Height:", imageActualHeight); // Debugging

            if (textActualHeight > imageActualHeight) {
                readMoreBtn.style.display = 'block'; // Show button if text is taller
            } else {
                readMoreBtn.style.display = 'none'; // Hide button otherwise
            }
        });
    }

    function addReadMoreEventListeners() {
        document.querySelectorAll('.row.pb-50').forEach(row => {
            let textContent = row.querySelector('.text-content1');
            let readMoreBtn = row.querySelector('.toggle-btn1');

            if (!textContent || !readMoreBtn) return; // Prevent errors
    ...
    document.addEventListener('DOMContentLoaded', () => {
        toggleReadMoreButtons(); // Delay execution for accuracy
        addReadMoreEventListeners(); // Attach event listeners after DOM is ready
    });

    window.addEventListener('resize', toggleReadMoreButtons);
